...
export class BinaryTranslator{
    static LabelToIndex(incomingStringValue = "", potentialOptions = []){
        ...
        if (!potentialOptions.includes(incomingStringValue)){
            console.error(`${incomingStringValue} is not included in potential options ${potentialOptions}`);
            return null;
        }
    ...
    static LabelsToIndexes(incomingData = [], potentialOptions = []){
        let modifiedData = []
        incomingData.forEach(d => modifiedData.push(this.LabelToIndex(d, potentialOptions)));
        return modifiedData;
    }

    static IndexToLabel(incomingBinaryData = [], potentialOptions = []){
        if (incomingBinaryData.length !== potentialOptions.length){
            console.error(`incoming binary data ${incomingBinaryData[0]}:${incomingBinaryData.length} not same length as ${potentialOptions.length}`)
            return null;
        }
    ...
    static IndexesToLabels(incomingBinaryMatrix = [], potentialOptions = []){
        ...
        incomingBinaryMatrix.forEach(b => dataArray.push(this.IndexToLabel(b, potentialOptions)));
    ...
    static IndexesToLabelsTensor(incomingBinaryMatrix = [], potentialOptions = []){
        let labels = this.IndexesToLabels(incomingBinaryMatrix, potentialOptions);
...
export class LinearTranlator{
    ...
    static IndexToLabel(incomingNumberedIndex = -1, potentialOptions = []){
        if (incomingNumberedIndex < 0 || incomingNumberedIndex >= potentialOptions.length){
